...
// `onPersist` 保存在 $DSH_HOME/dsh-bridge-gateway/config.json。本服务不直接碰文件系统。

import { Platform } from '../platform/base.js'
import { applySessionConfig, sessionConfigPatch } from '../platform/session-config.js'
...
export class WechatService extends Platform {
  ...
  /** 更新运行时配置（心跳间隔/审批超时/每气泡字数/分块延迟 + 会话级配置）并持久化。 */
  async setConfig({ digestIntervalSec, approvalTimeoutSec, maxMessageChars, sendChunkDelayMs, ...sessionPatch } = {}) {
    ...
    // 会话级配置（cwd / agentPreset / agentProvider / agentModel）：
    // 必须一并写入并持久化，否则「设置页配了、重启就丢」。
    applySessionConfig(this.node.config, sessionPatch)
    await this.persist({
      digestIntervalSec:  this.node.config.digestIntervalSec,
      approvalTimeoutSec: this.node.config.approvalTimeoutSec,
      maxMessageChars:    this.node.config.maxMessageChars,
      sendChunkDelayMs:   this.node.config.sendChunkDelayMs,
      ...sessionConfigPatch(this.node.config),
    })
  }
}
