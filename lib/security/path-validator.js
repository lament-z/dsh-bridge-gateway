...
export async function isSafeWorkspacePath(targetPath, options = {}) {
  ...
  // 2. Windows 盘符绝对路径（<盘符>:[\])：跨平台识别并应用系统目录黑名单。
  //    即使当前非 Windows，也拦截用户传入的 Windows 系统敏感路径（如 C:\Windows），
  //    避免在 POSIX 上被 resolve() 拼成相对路径后仅返回 ENOENT 而绕过黑名单。
  const winDriveMatch = trimmed.match(/^([A-Za-z]):[\\/](.*)$/);
  if (winDriveMatch) {
    const rest = winDriveMatch[2].toLowerCase();
    for (const prefix of WIN_SYSTEM_PREFIXES) {
      if (rest === prefix || rest.startsWith(`${prefix}\\`) || rest.startsWith(`${prefix}/`)) {
        return { valid: false, error: `安全拦截：禁止访问系统敏感目录 (${prefix})` };
      }
    }
  }

  // 3. 检查 Windows 敏感路径（在当前为 Windows 时）
  if (isWin) {
    // 提取盘符后面的相对部分
