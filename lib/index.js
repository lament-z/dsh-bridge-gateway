  ...
  async start() {
    ...
    const handler = (req, res) => {
      ...
      const proxyReq = httpRequest(
        ...
        (proxyRes) => {
          // DSH 的 index HTML 不带 cache-control，浏览器会**启发式缓存**它；而它里面钉着
          // 各客户端模块的 rev，于是 host 重启换了新 rev 之后，旧标签页/普通刷新仍指向旧 rev
          // 的 immutable bundle —— 表现就是"改了但页面永远不变"（远程端尤其明显）。
          // 这里对 HTML 响应强制 no-store：每次刷新都重新取 index。bundle 本身仍按 rev 长缓存。
