...
function gatewayDataDir() {
  return join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'dsh-bridge');
}
...
class ProxyServer {
  ...
  connectionSummary() {
    ...
    for (const conn of this.wsSockets.values()) {
      const item = byIp.get(conn.ip) ?? { ip: conn.ip, count: 0, via: conn.via, oldestSec: 0 };
      item.count += 1;
 ...
 */
class BridgeService {
  constructor({ dshPort, proxyPort, home, cloudflaredConfig, customTunnelConfig, lanConfig, gatewayConfig, authManager, onPersist, logger }) {
  ...
  async startGateway() {
    ...
    const certDir = join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'dsh-bridge', 'certs', `gateway-${port}`);
