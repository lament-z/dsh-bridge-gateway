...
// 通过回调 `onPersist` 保存在 $DSH_HOME/dsh-bridge-gateway/config.json。
...
export class QqService extends Platform {
  ...
  /** 更新运行时配置并持久化。 */
  async setConfig({ digestIntervalSec, approvalTimeoutSec, maxMessageChars, sendChunkDelayMs, appId, clientSecret, ...sessionPatch } = {}) {
    ...
    // 会话级配置（cwd / agentPreset / agentProvider / agentModel）：一并写入并持久化
    ...
    await this.persist({ ...patch, ...sessionConfigPatch(this.node.config) })
