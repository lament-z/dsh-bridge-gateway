...
export class CustomTunnelClient {
  ...
  _handleWsOpen(msg) {
    ...
    let headerBuf = '';
    let upgraded = false;
    // 握手后的 WebSocket 帧解析缓冲：裸 TCP 转发不会自动回 Pong，
    // 必须自己在帧层拦 Ping 并回 Pong（见 _processWsFrames）。
    let frameBuf = Buffer.alloc(0);

    sock.on('data', (chunk) => {
      if (upgraded) {
        frameBuf = this._processWsFrames(wsId, Buffer.concat([frameBuf, chunk]), sock);
        return;
      }
      ...
      // 握手后紧跟的帧数据（同样要过帧解析，可能直接就是个 Ping）
      const rest = headerBuf.slice(sep + 4);
      if (rest.length > 0) {
        frameBuf = this._processWsFrames(wsId, Buffer.from(rest, 'binary'), sock);
      }
    });
  ...
  // ── WebSocket 帧解析 ───────────────────────────────────────────────────────
  // 在裸 TCP 层面解析 WebSocket 帧，拦截 Ping (0x9) 自动回复 Pong (0xA)，其余帧原样转发。
  // 为什么必须自己做：隧道是裸 TCP 字节转发，WebSocket 的 Ping/Pong 属于协议层控制帧，
  // 不会被"应用层"自动应答；而 DSH API Gateway 每 2s 发一次 Ping，收不到 Pong 就会
  // terminate 连接 —— 表现为隧道频繁断连、会话中断。
  // 返回未消费完的残留缓冲，供下一个 chunk 拼接。
  _processWsFrames(wsId, buf, sock) {
    while (buf.length >= 2) {
      ...
      if (opcode === 0x9) {
        // Ping → 自动回复 Pong（同 payload）。
        // 必须加 mask：WebSocket 协议规定 client→server 方向的帧必须 mask，
        // 否则服务端 ws 库判定协议错误（1002）并关闭连接 —— 修好一个断连反而制造另一个。
