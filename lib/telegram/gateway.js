...
export function createConnectProxyAgent(proxyUrl) {
  ...
  // Node >=24 下把自定义 createConnection 作为 https.Agent 的构造参数会被静默忽略
  // （https.Agent 不拷贝构造入参里的 createConnection，回退到原型上的默认直连方法），
  // 结果是"配了代理却永远走直连"，且不报任何错，排查极其困难。
  // 改为构造完成后赋值**实例自有属性**：自有属性会正确覆盖原型方法，Node 22/24 均生效。
