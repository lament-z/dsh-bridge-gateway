...
const GatewayCard = React.memo(function GatewayCard({ gw, onStart, onStop, onSaveConfig, onToggleAutoStart }) {
  ...
  const handleSave = async (e) => {
    ...
    if (!Number.isInteger(p) || p < 1 || p > 65535) {
      setMsg({ ok: false, text: '端口需为 1-65535 之间的整数' });
      return;
    }
    setSaving(true);
    setMsg(null);
    try {
      ...
      setMsg({ ok: true, text: '✓ 端口配置已保存，需「开启」或重启后生效' });
    } catch (err) {
      setMsg({ ok: false, text: err.message || '保存失败' });
    } finally {
      setSaving(false);
  ...
  return React.createElement('div', { style: s.card },
    ...
    React.createElement('div', { style: s.block, fontSize: 12, lineHeight: 1.7, color: 'var(--dsw-alias-label-secondary,#6b7280)' },
      React.createElement('div', { style: { marginBottom: 6 } },
        '✅ 公网直达：' ,
        !running && React.createElement('span', null, '在路由器/云服务器上把端口 ', '0.0.0.0:', React.createElement('b', { style: { color: 'var(--dsw-alias-label-primary,#111827)' } }, port), ' 映射到本机后，公网可访问。'),
        running && React.createElement('span', null, '已监听 0.0.0.0:', React.createElement('b', { style: { color: 'var(--dsw-alias-label-primary,#111827)' } }, port), '，访问 https://<公网IP或域名>', port === 443 ? '' : `:${port}`),
      ),
      React.createElement('div', { style: { marginBottom: 6 } },
    ...
    React.createElement('form', { onSubmit: handleSave, style: { ...s.block, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', borderTop: '1px solid var(--dsw-alias-border-secondary,#e5e7eb)', paddingTop: 10 } },
      ...
      React.createElement('button', {
        ...
        title: running ? '运行中请先关闭再改端口' : '',
      }, saving ? '保存中…' : '保存端口'),
      msg && React.createElement('span', {
        style: { fontSize: 12, color: msg.ok ? 'var(--dsw-alias-state-success-primary,#059669)' : 'var(--dsw-alias-state-error-primary,#dc2626)' },
      }, msg.text),
    ),
    React.createElement('div', { style: { ...s.block, display: 'flex', gap: 8, flexWrap: 'wrap' } },
      !running && onStart && React.createElement('button', { style: s.btnPri, onClick: onStart }, '开启直连网关'),
      running && onStop && React.createElement('button', { style: s.btnGhost, onClick: onStop }, '关闭直连网关'),
    ),
  );
});
...
function BridgePanel({ rpcCall }) {
  ...
  const saveGatewayConfig = React.useCallback(({ port, autoStart }) =>
    act(BRIDGE_ENDPOINTS.gatewaySaveConfig, { port, autoStart })
  , [act]);
  const onToggleGatewayAutoStart = React.useCallback((autoStart) =>
    act(BRIDGE_ENDPOINTS.gatewaySetAutoStart, { autoStart })
  , [act]);
...
function injectMobileStyles() {
  ...
  style.textContent = `
    ...
    @media (max-width: 768px) {
      ...
      div[class*="_7KE1Ra_root"] > div[class*="menu"],
      div[class*="_7KE1Ra_root"] > div[class*="panel"],
      div[class*="Sh0Q9G_root"] > div[class*="dropdown"],
      ...
      div[class*="uV2eYG_overlayAnchor"] > div[role="menu"],
